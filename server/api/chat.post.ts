...
export default defineEventHandler(async event => {
  ...
  // 2. Берем из .env URL нашего чат-агента в n8n
  const agentUrl = process.env.N8N_CHAT_AGENT_URL;

  if (!agentUrl) {
    throw createError({
      statusCode: 500,
      statusMessage: "URL агента не настроен на сервере",
    });
  }
  ...
  try {
    ...
    const agentResponse = await $fetch<{ reply: string; sources?: string[] }>(
      agentUrl,
      {
        method: "POST",
